...
const CountrySelector = ({ disabled }) => {
  let [isCountryBttnClicked, setIsCountryBttnClicked] = useState(false);
  let [newCountriesList, setNewCountriesList] = useState([
  ...
  let [selectedCountry, setSelectedCountry] = useState(countries[117]);

  function handleSelectItem(country) {
    setSelectedCountry(country);
    setIsCountryBttnClicked(false);
    console.log(selectedCountry);
  }
  function filterCountries(countrysearch) {
    return countries.filter((country) =>
      country.name
        .toString()
        .toLowerCase()
        .startsWith(countrysearch.toString().toLowerCase())
    );
  }
  return (
    <div class="">
      <button
        type="button"
        class="bg-white flex justify-between w-full rounded-full border-0 my-2 px-4 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-green-500 sm:text-sm sm:leading-6"
        aria-haspopup="listbox"
        aria-expanded="true"
        aria-labelledby="listbox-label"
        disabled={disabled}
        onClick={() => {
          setIsCountryBttnClicked(!isCountryBttnClicked);
        }}
      >
        <span class="flex items-center">
          <img src={require(`../flags/${selectedCountry.alpha2}.png`)} alt="" />
      ...
      <div
        onPointerLeave={() => {
          setIsCountryBttnClicked(false);
          setNewCountriesList(countries);
        }}
        class={
          isCountryBttnClicked
            ? "  transition ease-in duration-100  absolute z-10 mt-1 max-h-56 w-5/6 md:w-1/2 overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm"
            : "hidden"
        }
      >
        <input
          class="h-9 pl-5 placeholder:text-gray-400 focus:outline-none focus:ring-0 w-full relative"
          placeholder="Search Country"
          type="text "
          onChange={(e) => {
            setNewCountriesList(filterCountries(e.target.value));
          }}
        />
        ...
        <ul
          class={
            isCountryBttnClicked
              ? "transition ease-in duration-100 opacity-100 opacity-0 relative z-10 mt-1 max-h-56 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm"
              : "hidden"
          }
          tabindex="-1"
          role="listbox"
        ...
        >
          {newCountriesList.length === 0 ? (
            <li class="text-gray-900 cursor-default select-none relative py-2 pl-3 pr-9">
              No countries found
            </li>
          ) : (
            newCountriesList.map((country) => (
              <li
                class="text-gray-900 relative cursor-default select-none py-2 pl-3 pr- hover:bg-green-500 hover:text-white"
                id="listbox-option-0"
                onClick={() => {
              ...
              >
                <div class="flex items-center">
                  <img src={require(`../flags/${country.alpha2}.png`)} alt="" />
                ...
                {selectedCountry.alpha2 === country.alpha2 ? (
                  <span class=" text-green-500 absolute inset-y-0 right-0 flex items-center pr-4">
                    <svg
                      class="h-5 w-5"
                      viewBox="0 0 20 20"
                      fill="currentColor"
